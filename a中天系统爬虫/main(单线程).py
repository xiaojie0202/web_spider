...
class ZTO(object):
    ...
    def inner_task(self):
        try:
            ...
            # 先通过短信接口获取， 失败的则添加到erro列表中
            # 再通过原生接口获取
            for dh in export_dh_list:
                d, info = func(zto_session, session, dh)
                if info == "需要登录":
                    messagebox.showwarning('ZTO_SESSION失效', 'ZTO_SESSION失效!')
                    return
                if info == "SESSION失效":
                    messagebox.showwarning('SESSION失效失效', 'SESSION失效失效!')
                    return
    ...
    def get_info(self, zto_session, session, dh):
        ...
        try:
            ...
            try:
                response = requests.get(url='https://newbill.zt-express.com/order-query/get?billCode=%s' % dh, headers=headers_2, cookies=cookies, timeout=5)
                print(dh, response.text)
            except TooManyRedirects as e:
                self.data_erro.append([dh, 'SESSION失效'])
                self.info.insert('', 0, values=(dh, 'SESSION失效'))
                return dh, 'SESSION失效'
            # {"server":"192.168.13.24","status":true,"statusCode":200} 暂无数据
            data2 = json.loads(response.text)
            if data2['status']:
                if data2.get('result', None):
                    ii = data2['result'][0]['receiveInfo'].split('-')[-1].split()[-1]
    ...
    def get_info_msg(self, zto_session, session, dh):
        try:
            ...
            try:
                response = requests.post(url='https://sms.zto.com/mobile/billcode', json=data, cookies=cookies, timeout=5)
            except TooManyRedirects as e:
                self.data_erro.append([dh, 'SESSION失效'])
                self.info.insert('', 0, values=(dh, 'SESSION失效'))
                return dh, 'SESSION失效'
...
if __name__ == '__main__':
    ZTO()
    # 65b170638803b957c06c3449d52f5c75666e64b5
    # 9639da6f-ca94-4e09-8f68-b75f451be2a0